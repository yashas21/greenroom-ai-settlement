...
export function CardTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return (
    <h3
      className={cn("text-[13px] font-semibold text-ink-900 tracking-tight", className)}
      {...props}
    />
  );
}
...
export function CardFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn("px-5 py-3 border-t border-ink-100/80 text-[12px] text-ink-500", className)}
      {...props}
    />
  );
}
...
export function Field({
  ...
  return (
    <div className={className}>
      ...
      <div className={cn("text-[13.5px] text-ink-900", mono && "font-mono tabular")}>
        {value}
      </div>
    </div>
  );
