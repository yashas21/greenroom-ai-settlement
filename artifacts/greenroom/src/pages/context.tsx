...
import { ArrowLeft, ArrowRight, FileText, Users, AlertCircle } from "lucide-react";
    ...
    <div className="px-12 py-10 max-w-3xl">
      ...
      <div className="rounded-lg border border-brand-200/60 bg-brand-50/30 p-5 flex gap-3 mb-12">
        ...
        <p className="text-[13px] text-ink-800 leading-relaxed">
          <strong className="font-semibold">Read your case study brief first.</strong>{" "}
      ...
      <ol className="space-y-5 text-[13.5px] text-ink-600 leading-relaxed mb-14">
        <Step n={1}>
          Open{" "}
          <Link href="/shows" className="text-brand-700 font-medium hover:text-brand-800 hover:underline">
            /shows
          </Link>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-14">
        <DocLink icon={FileText} title="CEO memo" file="data/ceo-memo.md"
          desc="Pri's Q4 all-hands on craft vs. completeness. The strategic frame." />
        <DocLink icon={FileText} title="The dispute thread" file="data/dispute-thread.md"
          desc="The full email chain on the marketing-recoup ambiguity that cost The Crescent $720." />
        <DocLink icon={Users} title="User research transcripts" file="data/transcripts/"
          desc="Mariana, Diego (tour manager), Marcus (GM), and Sarah Kim (WME agent). Mine these." />
        <DocLink icon={FileText} title="README" file="README.md"
      ...
      <div className="mt-12 pt-6 border-t border-ink-200/60 flex items-center justify-between">
        ...
        <span className="text-[11.5px] text-ink-400">Welcome to The Crescent.</span>
      </div>
    </div>
  );
}
...
function DocLink({
  icon: Icon, title, file, desc,
}: {
  icon: React.ComponentType<{ className?: string }>;
  title: string; file: string; desc: string;
}) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        ...
        <p className="text-[12.5px] text-ink-600 mt-2 leading-relaxed">{desc}</p>
      </CardContent>
    </Card>
  );
}
