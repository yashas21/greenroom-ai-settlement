...
export default function ReportsPage() {
  const state = useApiData(() => api.reports(), []);
  ...
  const dealMix = Object.entries(r.dealTypeCounts)
    .map(([type, count]) => ({
      type, count, pct: r.totalDeals > 0 ? count / r.totalDeals : 0,
    }))
  ...
  const lifecycleData = lifecycleOrder
    .map((stage) => ({
      stage, count: r.settlementStatus[stage] ?? 0,
      pct: r.totalSettlements > 0
        ? (r.settlementStatus[stage] ?? 0) / r.totalSettlements : 0,
    }))
    .filter((d) => d.count > 0);

  const maxLifecycleCount = Math.max(...lifecycleData.map((d) => d.count), 1);
  ...
  return (
    <div className="px-12 py-10 max-w-7xl">
      ...
      <div className="rounded-xl border border-amber-200/60 bg-gradient-to-r from-amber-50/60 to-canvas p-6 flex gap-4 mb-10">
        ...
        <div>
          <div className="eyebrow text-[10px] text-amber-800 mb-2">From Pri&apos;s Q4 memo</div>
      ...
      <div className="mb-16">
        <h2 className="font-display text-[28px] font-medium text-ink-900 mb-8" style={{ letterSpacing: "-0.02em" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="relative overflow-hidden rounded-xl border border-amber-200/60 bg-gradient-to-br from-amber-50/50 to-canvas p-8">
            ...
            <div className="relative">
              ...
              <div className="text-[64px] font-mono tabular font-bold text-amber-800 leading-none" style={{ letterSpacing: "-0.03em" }}>
                {unsupportedPct}%
              </div>
          ...
          <div className="relative overflow-hidden rounded-xl border border-rose-200/60 bg-gradient-to-br from-rose-50/40 to-canvas p-8">
            ...
            <div className="relative">
              ...
              <div className="text-[64px] font-mono tabular font-bold text-rose-800 leading-none" style={{ letterSpacing: "-0.03em" }}>
                {disputedPct}%
              </div>
              <p className="text-[12.5px] text-ink-600 mt-4 leading-relaxed max-w-sm">
      ...
      <div className="mb-16">
        <h2 className="font-display text-[24px] font-medium text-ink-900 mb-2" style={{ letterSpacing: "-0.02em" }}>
        ...
        <div className="space-y-[6px]">
          {lifecycleData.map(({ stage, count, pct }) => {
            const isProblem = stage === "disputed" || stage === "revised" || stage === "voided";
            const isDone = stage === "paid";
            const barWidth = (count / maxLifecycleCount) * 100;
            return (
                ...
                <div className="flex-1 flex items-center gap-2">
                  <div
                    className={`h-7 rounded-[4px] transition-all duration-300 flex items-center min-w-[28px] ${
                      isProblem ? "bg-rose-500/90" : isDone ? "bg-brand-700/90" : "bg-sky-500/80"
                    }`}
      ...
      {r.settlementsWithRecoups > 0 && (
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            />
            <SmallMetric label="Total recouped" value={formatMoneyCompact(r.totalRecoupValue)} mono />
            <SmallMetric
        ...
        <Card>
          <CardContent>
            <div className="grid grid-cols-3 gap-6 mb-6 pb-5 border-b border-ink-100/60">
              <div>
                <div className="eyebrow text-[10px] text-ink-400 mb-1">Total comp tickets</div>
                <div className="text-[24px] font-mono tabular font-semibold text-ink-900">
                  {r.totalCompTickets.toLocaleString()}
                </div>
              </div>
              <div>
                <div className="eyebrow text-[10px] text-ink-400 mb-1">Face value foregone</div>
                <div className="text-[24px] font-mono tabular font-semibold text-ink-900">
                  {formatMoneyCompact(r.totalCompFaceValue)}
                </div>
              </div>
              <div>
                <div className="eyebrow text-[10px] text-ink-400 mb-1">Per show (avg)</div>
                <div className="text-[24px] font-mono tabular font-semibold text-ink-900">
                  {Math.round(r.totalCompTickets / Math.max(r.showCount, 1))}
                </div>
              </div>
            </div>
            <div className="space-y-[6px]">
              {Object.entries(r.compsByCategory)
                .sort(([, a], [, b]) => b - a)
                .map(([cat, count]) => {
                  const pct = r.totalCompTickets > 0 ? count / r.totalCompTickets : 0;
                  const maxCount = Math.max(...Object.values(r.compsByCategory), 1);
                  ...
                  return (
                    <div key={cat} className="flex items-center gap-3">
                      <div className="w-32 text-right">
                        <span className="text-[12px] text-ink-600">{labels[cat] ?? cat}</span>
                      </div>
                      <div className="flex-1 flex items-center gap-2">
                        <div
      ...
      <div className="mb-10">
        <h2 className="font-display text-[24px] font-medium text-ink-900 mb-5" style={{ letterSpacing: "-0.02em" }}>
          Deal mix
        </h2>
        <Card>
          <CardContent>
            <div className="space-y-[6px]">
              {dealMix.map(({ type, count, pct }) => {
                const supported = type === "flat" || type === "percentage_of_gross";
                const maxCount = Math.max(...dealMix.map((d) => d.count), 1);
                const barWidth = (count / maxCount) * 100;
...
function SmallMetric({
  label, value, subtext, mono = false, alarming = false,
}: {
  label: string; value: string; subtext?: string; mono?: boolean; alarming?: boolean;
}) {
  return (
    <div className={`rounded-lg border p-5 ${alarming ? "border-rose-200/60 bg-rose-50/20" : "border-ink-200/60 bg-white"}`}>
      <div className={`eyebrow text-[10px] ${alarming ? "text-rose-700" : "text-ink-400"}`}>
        {label}
      </div>
      <div className={`text-[24px] font-semibold mt-1.5 leading-none ${alarming ? "text-rose-700" : "text-ink-900"} ${mono ? "font-mono tabular" : ""}`}>
        {value}
      </div>
      {subtext && <div className="text-[11px] text-ink-400 mt-2">{subtext}</div>}
    </div>
  );
}
